...
void popAndEval(OperatorStack*& opStack, ValueStack*& valStack){
  char op = opStack->top();
  opStack->pop();
  int v2 = valStack->top();
  valStack->pop();
...
void processExpression (Token inputToken, TokenReader *tr)
{
 ...
 OperatorStack* oStack = new OperatorStack();
    ...
    /* The expression contains an OPERATOR */
    else if (inputToken.equalsType(OPERATOR))
      {
        ...
        if(inputToken.getOperator() == '+' || inputToken.getOperator() == '-'){
          while(!oStack->isEmpty() && ((oStack->top() == '*') || (oStack->top() == '/') || (oStack->top() == '+') || (oStack->top() == '-') || (oStack->top() == '%'))){
            popAndEval(oStack, vStack);
          }
          oStack->push(inputToken.getOperator());

        }
        if(inputToken.getOperator() == '*' || inputToken.getOperator() == '/' || inputToken.getOperator() == '%'){
          while(!oStack->isEmpty() && ((oStack->top() == '*') || (oStack->top() == '/') || (oStack->top() == '%'))){
            popAndEval(oStack, vStack);
          }
          oStack->push(inputToken.getOperator());
        }
        if(inputToken.getOperator() == ')'){
          while(!oStack->isEmpty() && oStack->top() != '('){
            popAndEval(oStack, vStack);
          }
          if(oStack->isEmpty()){
            printf("Invalid expression.");
